/****** OBJAPI.H ************* Shishkova L.A.  ******** 04/05/09 ***
******* OBJAPI.H ************* Belenkov  O.V.  ******** 09/03/16 ***
******* OBJAPI.H ************* Shabakov D.A.   ******** 31/05/13 ***
*                                                                  *
...
********************************************************************
*                                                                  *
*               ОПИСАНИЕ КЛАССА ДОСТУПА К ОБЪЕКТУ                  *
...
typedef struct OBJECTFORM      // ПАРАМЕТРЫ ОТОБРАЖЕНИЯ ИНФОРМАЦИИ ОБ ОБ'ЕКТЕ
{
  ...
  POINT       Position;        // Положение курсора в точке выбора в координатах экрана        //28/11/02
  POINT       LeftTopPosition; // Координаты левого верхнего угла карты в пикселах в районе    //28/11/02
  long int    PlaceSystem;     // Текущая система отображения координат метрики объекта:
                               ...
                               // 7 - в радианах (общеземной эллипсоид WGS84)                    //28/01/05
...
typedef HANDLE  HCHOICEOBJECT; // ИДЕНТИФИКАТОР ОТКРЫТОГО СЕАНСА ВЫБОРА ОБЪЕКТА
...
extern "C"
{
 ...
 // (для LINUXAPI система координат только в дискретах)      // 12/02/03
 ...
 //================================================================
 // Выполнение НЕМОДАЛЬНОГО диалога выбора объекта                       // 04/05/09
 ...
 // Завершение процесса выбора объекта
 // Всегда возвращает 1

 long int _export WINAPI
      formDestroySelectObject(HCHOICEOBJECT choiceobject);

 // При закрытии диалога выбора объекта посылается сообщение         // 04/05/09
 // с кодом 3339 окну документа (objform.Window), в котором
 // первый параметр равен
 // 1 - если диалог закрывается нажатием клавиши "Выбрать"
 // 0 - если диалог закрывается другим способом
 // Второй параметр содержит HOBJ объекта
 // При обработке cообщения и требуется выполнить
 // функцию formDestroySelectObject(HCHOICEOBJECT choiceobject);
 ...
 // flag   - порядок поиска объектов (WO_FIRST, WO_NEXT..., см. maptype.h)
