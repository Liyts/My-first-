/******  MAPCREAT.H  ***********  Belenkov  O.V. *****  06/12/16 ***
*******  MAPCREAT.H  ***********  Shishkova L.A. *****  15/02/12 ***
*******  MAPCREAT.H  ***********  Tarakanova I.R.*****  30/09/16 ***
*******  MAPCREAT.H  ***********  Борзов А.Ю.    *****  09/09/16 ***
*******  MAPCREAT.H  ***********  Dishlenko S.G. *****  26/03/12 ***
*******  MAPCREAT.H  ***********  Gorbunov  A.V. *****  29/09/16 ***
*                                                                  *
...
// а параметры связи с глобальной системой координат (геодезической) отсутствуют.
// Например, поэтажный план, план участка и т.п.
//
// Выбор типа карты не влияет на работу функций определения геодезических
// координат, расчета длин, площадей и азимутов. За исключением точек, существенно удаленных
// от осевого меридиана топографической карты, для проекций UTM и Гаусса-Крюгера.
...
// UNDEFINED                   = -1, // Не установлено
   ...
   LAMBERTAZIMUTHALEQUALAREA   = 24, // Азимутальная (нормальная) равновеликая
                                     // проекция Ламберта (то же, что и 30) 
   ...
   LASTPROJECTIONNUMBER        = 39, // Крайний номер проекции
  }
   MAPPROJECTION;
  ...
  //   "Miller Cylindrical"               // 34 - ESRI:54003
  //   "World Mercator"                   // 35 - EPSG:3857
  //   "MERCATOR_2SP"                     // 36 - EPSG:9805
  //   "MODIFIEDAZIMUTALEQUIDISTANT"      // 37 - EPSG:9832
  //   "AZIMUTALEQUIDISTANTGUAM"          // 38 - EPSG:9831
  //   "DOUBLESTEREOGRAPHIC"              // 39 - EPSG:9809 
...
// UNDEFINED   = -1,       // Не установлено
   ...
   HEIGHTSYSTEMCOUNT = 29  // Количество систем высот
  }
   HEIGHTSYSTEM;
...
// UNDEFINED          = -1,  // Не установлено
  ...
  // Типы флага "Вид рамки"  (НЕ ИСПОЛЬЗУЕТСЯ)
...
typedef struct MAPREGISTEREX
{
  ...
  int         CoordinateSystem       ; // Система координат (устаревшее)
  ...
  double      FirstMainParallel      ; // Первая главная параллель  StandardParallel1 в радианах
  double      SecondMainParallel     ; // Вторая главная параллель  StandardParallel2 в радианах
  double      AxisMeridian           ; // Осевой меридиан (Долгота полюса проекции) CentralMeridian в радианах
  double      MainPointParallel      ; // Параллель главной точки (Широта полюса проекции) LatitudeOfOrigin в радианах
  double      PoleLatitude           ; // (Latitude of false origin, etc) в радианах
  double      PoleLongitude          ; // (Longitude of false origin, etc) в радианах
  double      FalseEasting           ; // Смещение координат по оси Y в метрах
  double      FalseNorthing          ; // Смещение координат по оси X в метрах
  double      ScaleFactor            ; // Масштабный коэффициент на осевом меридиане (1.0 +\- ...)
  double      TurnAngle              ; // Угол разворота осей для локальных систем (МСК) в радианах (устаревшее, см. LOCALTRANSFORM)
  double      Reserv2[4]             ; // Резерв = 0
}
  MAPREGISTEREX;


typedef struct MAPREGISTER             // УСТАРЕВШАЯ СТРУКТУРА  
{
  int         Length                 ; // Размер данной структуры
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// Параметры геодезической системы координат в базе данных EPSG   
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// Параметры прямоугольной системы координат в базе данных EPSG 
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// Единицы измерения в базе данных EPSG                     
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
typedef struct OFFSETSCALEROTATE
{
  ...
  double dX, dY; // смещение по X, Y
}
  OFFSETSCALEROTATE;

// The parameters for coordinate transformation             // 09/09/16
